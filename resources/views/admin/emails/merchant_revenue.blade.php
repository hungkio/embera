...
<div class="container">
    ...
    <div class="header">
        ...
        <div class="bm">BM: ....</div>
        <div class="address">Số 166 Phạm Văn Đồng, Phường Xuân Đỉnh, Quận Bắc Từ Liêm, Thành phố Hà
            Nội, Việt Nam
        </div>
    </div>
    ...
    <div class="section">
        Hôm nay, ngày {{ $content['hom_nay_ngay'] ?? '' }} tháng {{ $content['hom_nay_thang'] ?? ''
        }} năm {{ $content['hom_nay_nam'] ?? '' }}, chúng tôi gồm có:<br>
        <strong>BÊN A: CÔNG TY CỔ PHẦN CÔNG NGHỆ EMBERA</strong><br>
        <strong>BÊN B: {{ $content['ben_b'] ?? '' }}</strong><br><br>
        Căn cứ theo Hợp đồng số {{ $content['hop_dong_so'] ?? '' }} ký giữa Bên A và Bên B, hai bên
        cùng thống nhất lập biên bản xác nhận doanh thu chia sẻ như sau:
    </div>

    <div class="section">
        <strong>· Kỳ xác nhận:</strong> Từ ngày {{ $content['from_day'] ?? '' }} tháng
        {{ $content['from_month'] ?? '' }} năm {{ $content['from_year'] ?? '' }} đến ngày
        {{ $content['to_day'] ?? '' }} tháng {{ $content['to_month'] ?? '' }} năm
        {{ $content['to_year'] ?? '' }}
    </div>
    ...
    <div class="section">
        ...
        {{ $content['tong_thanh_toan_share'] ?? '' }} ({{ $content['tong_thanh_toan_text'] ?? '' }}
        )<br>
    ...
    <div class="footer">
        Biên bản này được gửi qua email và có hiệu lực như một biên bản xác nhận chính thức. Biên
        bản được lưu trữ cùng thông tin gửi email để làm căn cứ xác nhận giữa hai bên.<br>
        Trong vòng 05 (năm) ngày kể từ ngày biên bản này được gửi, nếu Bên B không có phản hồi chính
        thức bằng văn bản hoặc email, thì được hiểu là Bên B đã đồng ý toàn bộ nội dung biên bản
        cũng như các điều khoản thanh toán nêu trên.
    </div>
